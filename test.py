...
Original file is located at
    https://colab.research.google.com/drive/1tupGM9o9BAJCfdN7q53CWYhOOxkTkobj
"""
...
def q_algo(game, playerA, playerB):
    ...
    for i in range(iterations):
        ...
        Qa[a, b, ball, actionA] = (1 - alpha) * Qa[a, b, ball, actionA] + \
                                  alpha * ((1 - gamma) * rewardA + gamma * np.max(Qa[na, nb, nball]))
        # print(Qa[a, b, ball, actionA], rewardA, np.max(Qa[na, nb, nball]))
...
def ce_q(game, playerA, playerB):
    ...
    for i in range(iterations):
        ...
        #Update the value only if it changed during the iteration
        # print(Qa[a, b, ball, actionA, actionB], rewardA, r_exp_A)
        if [a, b, ball, actionA, actionB] == [2, 1, 1, 2, 4]:
            error_list.append(abs(Qa[2, 1, 1, 2, 4] - prev_q_val))
            iter_list.append(i)
            print("Iteration: ", i, abs(Qa[2, 1, 1, 2, 4] - prev_q_val))
